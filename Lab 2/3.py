...
class environment:
    def __init__(self, velocity):
        ...
        for i in range(no_of_bots):
            agents.append(Mycircle(euclid.Vector2(bots[i][0], bots[i][1]), size, colors[random.randint(0, len(colors)-1)], velocity, 1))
        ...
        centx /= no_of_bots
        centy /= no_of_bots
        final_position = euclid.Vector2(centx, centy)
        flag1 = True
        time2 = 0
        while flag1:
            time1 = 0
            ...
            while flag and flag1:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        flag1 = False
                for i in range(no_of_bots):
                    position = AgentFunction(agents[i].position, velocity, timediff, final_position)
                    if collisionChecking(agents, i):
                        agents[i].move(position)
                pygame.display.flip()
...
def collisionChecking(agents, current):
    for i in range(len(agents)):
        if not i == current:
            diff = agents[i].position-agents[current].position
